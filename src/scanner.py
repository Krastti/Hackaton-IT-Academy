...
class PIIAnalyzer:
    ...
    def _deduplicate_ids(self, items: set, threshold: float = 0.85) -> set:
        ...
        return set(visual_items) if (mrz_items and visual_items) else set(unique_items)
    ...
    def _extract_names_from_mrz(self, text: str) -> set:
        ...
        if mrz_match:
            ...
            if clean_surname and clean_given:
                ...
                names.add(f"{clean_surname.translate(tr_map)} {clean_given.translate(tr_map)}".title())
    ...
    def _extract_foreign_pii_by_anchors(self, text: str) -> Dict[str, set]:
        ...
        doc_nos = re.findall(r'(?i)\b(?:document no|passport no|id no|identity no|document number|номер документа)[\s:;\.\|№#]{1,20}([A-Z0-9]{6,14})\b', text)
        for doc in doc_nos:
    ...
    def _heal_ocr_digits(self, text: str) -> str:
        for old, new in {'O':'0', 'о':'0', 'О':'0', 'З':'3', 'з':'3', 'B':'8', 'В':'8', 'I':'1', 'l':'1', 'S':'5'}.items():
            text = text.replace(old, new)
        return text

    def _process_match(self, key: str, match: str) -> Optional[str]:
        if key == 'bank_card': return normalize_gov_id(match) if is_luhn_valid(match) else None
        if key == 'snils':
            norm = normalize_gov_id(match)
            return norm if is_snils_valid(norm) else None
        if key == 'passport':
            cleaned = re.sub(r'[^A-Za-zА-Яа-я0-9]', '', re.sub(r'(?i)(паспортные данные|паспорт|серия и номер|серия|passport|document no\.?)', '', match))
            return cleaned if len(cleaned) >= 6 else None
        if key == 'inn': return normalize_gov_id(match)
        ...
        for name in fio_candidates.values():
            for i, ext in enumerate(final_fios):
                if difflib.SequenceMatcher(None, name.lower(), ext.lower()).ratio() > 0.6:
                    if len(name) > len(ext): final_fios[i] = name
                    break
            else:
    ...
    def analyze_text(self, text: str) -> Dict[str, Any]:
        text = text[:500000] if len(text) > 500000 else text
        ...
        if mrz_names: findings['fio'].update(mrz_names)
        elif anchor_data['fio']: findings['fio'].update(anchor_data['fio'])
        else: findings['fio'].update(self._extract_fios(text))

        if anchor_data['passport']: findings['passport'].update(anchor_data['passport'])
